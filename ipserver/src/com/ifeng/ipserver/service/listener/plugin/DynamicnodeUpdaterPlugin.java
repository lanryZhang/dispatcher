...
public class DynamicnodeUpdaterPlugin implements IntfPlugin,Configurable {
	...
	//	filepath = filepath.substring(0,filepath.lastIndexOf("/"))+"/server.xml";
		...
		try{
			...
			while(it.hasNext()){
				...
				DynamicNewNode node = (DynamicNewNode) entry.getValue();
				if(!dynamicMap.containsKey(nodeid)){ //如果原来不存在此节点，则添加到map中
					dynamicMap.put(nodeid, node);
				}else{ //如果有则将最新数据更新进去
					DynamicNewNode dyNode = (DynamicNewNode) dynamicMap.get(nodeid);
			...
			for(Object key:delListKey){
				DynamicNewNode tempNode = (DynamicNewNode) dynamicMap.get(key);
				tempNode.getUpdateRateTimer().cancel();
				tempNode.getRateCalculator().getUpdateRateTimer().cancel();
				dynamicMap.remove(key);
			}
		}catch(Throwable e){
			log.warn("nodeMap update failed:"+e);
			return Boolean.FALSE;
		}

		log.info("nodeMap is update");
		return Boolean.TRUE;
	}
	@Override
